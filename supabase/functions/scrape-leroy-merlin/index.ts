...
const LEROY_NAME = "Leroy Merlin";
const LEROY_URL = "https://www.leroymerlin.com.br";
...
async function scrapeMaterial(material: Material) {
  const searchUrl = `${LEROY_URL}/busca?q=${encodeURIComponent(material.nome)}`;
  ...
  if (!response.ok) throw new Error(`Leroy Merlin respondeu HTTP ${response.status}`);
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data: existingStores, error: storeLookupError } = await supabase.from("lojas").select("id").ilike("nome", LEROY_NAME).limit(1);
    if (storeLookupError) throw storeLookupError;

    let lojaId = existingStores?.[0]?.id as string | undefined;
    if (!lojaId) {
      const { data: loja, error: storeCreateError } = await supabase.from("lojas").insert({ nome: LEROY_NAME, cidade: "Online", contato: LEROY_URL }).select("id").single();
    ...
    return Response.json({ total: results.length, found, failed, delayMs: REQUEST_DELAY_MS, results }, { headers: { ...corsHeaders, "Content-Type": "application/json" } });
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
