...
export function extractLowestRelevantPrice(html: string, searchTerm: string): number | null {
  ...
  // Last resort for product grids that render prices directly in their HTML.
  if (candidates.length === 0) {
    for (const match of html.matchAll(/(?:R\$|R&#36;|R&dollar;)\s*([\d.]+,\d{2})/gi)) {
      const prefix = html.slice(Math.max(0, (match.index ?? 0) - 100), match.index).replace(/<[^>]*>/g, " ").toLocaleLowerCase("pt-BR");
      if (/(?:\d+\s*x\s*(?:de)?|parcela|mensais)\s*$/.test(prefix)) continue;
      const price = parsePrice(match[1]);
      if (price) candidates.push({ name: searchTerm, price });
    }
  }
